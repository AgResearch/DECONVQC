...
psql -q -U agrbrdf -d agrbrdf -h invincible -v keyfilename=\'$KEYFILE_BASE\' -f $GBS_BIN/database/extractKeyfile.psql

echo "do you want to continue - if you answer y the keyfile will be deleted from the database (but not from $KEY_DIR) (y/n)"
answer="n"
read answer
 if [ "$answer" != "y" ]; then
   echo "OK quitting"
...
if [ $DRY_RUN == "no" ]; then
   psql -q -U agrbrdf -d agrbrdf -h invincible -v keyfilename=\'$KEYFILE_BASE\' -f $GBS_BIN/database/deleteKeyfile.psql
else
